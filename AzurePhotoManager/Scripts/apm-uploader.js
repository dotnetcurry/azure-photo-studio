...
{
    $(document).on("click", "#uploadImages", beginUpload);
    ko.applyBindings(uploaders);
});

var beginUpload = function ()
{
    var fileControl = document.getElementById("selectImages");
...
var chunkedFileUploader =
{
    ...
    sendFile: function (uploader)
    {
        ...
        sendNextChunk = function ()
        {
            ...
            jqxhr = $.ajax({
                async: true,
                url: ('/Upload/UploadChunk?id=' + uploader.currentChunk + "&fileIndex=" + uploader.fileIndex),
                data: fileChunk,
                cache: false,
                contentType: false,
                processData: false,
